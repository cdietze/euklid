...
object GeometryUtil {
    ...
    fun intersectLines(x1: Float, y1: Float, x2: Float, y2: Float, x3: Float, y3: Float,
        ...
        // float comparison
        if (x3 == x4 && y3 == y4 && A1 * x3 + B1 * y3 + C1 == 0f && x3 >= min(x1, x2) &&
                x3 <= max(x1, x2) && y3 >= min(y1, y2) && y3 <= max(y1, y2)) {
            return 1
        }
        if (abs(coefParallel) < EPSILON) {
            return 0
        }
        point[0] = (B1 * C2 - B2 * C1) / coefParallel
        point[1] = (A2 * C1 - A1 * C2) / coefParallel
        if (point[0] >= min(x1, x2) && point[0] >= min(x3, x4) &&
                point[0] <= max(x1, x2) && point[0] <= max(x3, x4) &&
                point[1] >= min(y1, y2) && point[1] >= min(y3, y4) &&
                point[1] <= max(y1, y2) && point[1] <= max(y3, y4)) {
            return 1
        }
        return 0
    }
    ...
    private fun cubicNewton(xCoefs1: FloatArray, yCoefs1: FloatArray,
        ...
        while (sqrt(((t - t1) * (t - t1) + (s - s1) * (s - s1)).toDouble()) > EPSILON) {
